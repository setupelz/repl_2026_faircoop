...
Promise.all([
  ...
  ...["fig00", "figcap", "figeff", "figcoal", "fig04", "fignonco2", "figsec", "fig03", "fig01",
      "figlow", "figh2", "fig05", "figland", "figtrans"].map(id => d3.json(`data/${id}.json?v=${Date.now()}`)),
...
function openModal(doc) {
  ...
  <div class="srcblock">${doc.panels.map(p =>
    `<div><b>${p.title}</b> (${p.unit}): ${p.formed}. Indicator: ${p.indicator}.</div>`).join("")}
