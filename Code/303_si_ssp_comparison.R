...
#   d  group emission pathways, gross vs net, higher- vs lower-resp.
#   e  global sectoral benchmarks, Coal/Gas/Oil/Renewables vs 2020
...
# Main variants only: no CDR-scope, delay or discount-rate sensitivity runs.
scenario_sets_long <- scenario_sets_raw %>%
  filter(scenario_set == "800fm_ecpc2015",
         !grepl("CDR|Delay|Baseline", variant), !grepl("dr", model)) %>%
  mutate(ssp = ifelse(grepl("SSP_SSP1", model), "SSP1 2C", "SSP2 2C"),
         scenario_set = ssp) %>%
  prepare_long_format() %>%
  mutate(scenario_label = create_scenario_label(variant))
...
ssp_scales <- list(
  ...
  scale_shape_manual(values = state_shapes, breaks = variant_label_order,
...
proof <- scenario_sets_long %>%
  ...
  summarise(cml = trapz_integral(value, year) / MT_TO_GT, .groups = "drop") %>%
...
p_proof <- ggplot(proof, aes(scenario_set, cml, colour = scenario_set,
  ...
  geom_point(size = 2.6) +
  geom_point(data = function(d) filter(d, scenario_label == "Source"),
...
# --- b and c. Regional effort: higher-resp. excess and transfer volume --------
phys_region <- scenario_sets_long %>%
  ...
  summarise(coop_gt = step_integral(value, year) / MT_TO_GT, .groups = "drop")
...
alloc_gt <- scenario_sets_long %>%
  ...
  group_by(scenario_set, region) %>% summarise(alloc = first(value), .groups = "drop")

rem_ul <- scenario_sets_long %>%
  filter(variable == "Emissions|Allocation|Remaining domestic", year == 2110,
         grepl("ECPC", variant)) %>%
  transmute(scenario_set, scenario_label, region, rem = value)
rem_src <- scenario_sets_long %>%
  filter(variable == "Emissions|Allocation|Remaining domestic|Source",
         year == 2110, grepl("ECPC", variant)) %>%
  group_by(scenario_set, region) %>% summarise(rem = first(value), .groups = "drop") %>%
...
paths <- scenario_sets_long %>%
  ...
  mutate(pct = pct_vs(gt, year)) %>% ungroup() %>%
