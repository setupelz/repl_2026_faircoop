...
# ---------------------------------------------------------------------------
# (a) SI Table 2 — total transfers per approach and budget (main-text 10.1-44.8
#     range; 14.8->29.2 / 6.3->19.6 doubling under 1.5C). Six 800fm approaches
#     + 500fm ECPC2015, U and L corners, no CDR/Delay, 5% NPV.
# ---------------------------------------------------------------------------
bud_df <- scenario_sets_raw %>%
  filter(scenario_set %in% c(grid_sets, "500fm_ecpc2015"),
         grepl("SSP_SSP2", model), !grepl("dr", model),
         !grepl("CDR|Delay", variant)) %>%
  ...
  mutate(year = as.numeric(year), state = create_scenario_label(variant),
         lab = lab_of(ifelse(grepl("ecpc", scenario_set), "ECPC", "CAPC"),
                      stringr::str_extract(scenario_set, "1990|2015|2025")),
...
transfers_tbl <- coop_total(bud_df, 0.05) %>%
  tidyr::separate(lab, into = c("approach", "budget"), sep = " \\| ") %>%
  mutate(tier = ifelse(state == "Unlimited (U)", "U", "L")) %>%
  select(approach, budget, tier, transfers_tn = tn) %>%
  arrange(budget, approach, desc(tier))

# ---------------------------------------------------------------------------
# (b) SI Table 3 — CDR-scope cost-effectiveness ($ per tCO2 transferred), 800fm_ecpc2015
#     (main-text ~20-30 vs ~220 USD/tCO2; all values per-corner levels).
...
# ---------------------------------------------------------------------------
uc_df <- scenario_sets_raw %>%
  filter(scenario_set == "800fm_ecpc2015", grepl("SSP_SSP2", model),
         !grepl("dr", model),
         variant %in% c("U. SSP2-2C-ECPC2015", "L. SSP2-2C-ECPC2015",
...
uc_mit <- uc_df %>%
  filter(variable == "Transfers|Mitigation", region %in% hi,
         year >= 2030, year <= 2110) %>%
  group_by(lab, state, region) %>%
  summarise(gt = step_integral(value, year) / 1e3, .groups = "drop") %>%
  group_by(lab, state) %>%
  summarise(paid_gt = -sum(gt), .groups = "drop")  # higher-resp pay (negative) -> positive
unitcost_tbl <- uc_fin %>%
  left_join(uc_mit, by = c("lab", "state")) %>%
  tidyr::separate(lab, into = c("scope", NA), sep = " (?=Unlimited|Lowest)",
...
# ---------------------------------------------------------------------------
# (c) SI Table 4 — world consumption cost benchmarks (abstract 0.8% -> ~1.0% -> at most 1.5%;
#     economics-section spans; CDR-only 0.87% vs source)
...
# ---------------------------------------------------------------------------
cons_l <- scenario_sets_raw %>%
  filter(scenario_set %in% c(grid_sets, "500fm_ecpc2015"),
         grepl("SSP_SSP2", model), !grepl("dr", model),
         !grepl("CDR|Delay", variant), variable == "Consumption",
         region %in% c(hi, lo)) %>%
  pivot_longer(cols = matches("\\d{4}"), names_to = "year", values_to = "value") %>%
  mutate(year = as.numeric(year),
         state = ifelse(variant == "Baseline", "Baseline",
...
cons_cdr <- scenario_sets_raw %>%
  filter(scenario_set == "800fm_ecpc2015", grepl("SSP_SSP2", model),
         !grepl("dr", model), variable == "Consumption", region %in% c(hi, lo),
...
# ---------------------------------------------------------------------------
# (d) SI Table 5 — fossil decline benchmarks (abstract "up to 21% in 2040")
#     World Primary Energy|Fossil at 2030/2040/2050, lowest-f. corner vs the
#     unlimited corner and vs Source, per 800fm approach.
# ---------------------------------------------------------------------------
fossil_tbl <- scenario_sets_raw %>%
  filter(scenario_set %in% grid_sets, grepl("SSP_SSP2", model), !grepl("dr", model),
         !grepl("CDR|Delay", variant), variable == "Primary Energy|Fossil",
         region %in% c(hi, lo)) %>%
  pivot_longer(cols = matches("\\d{4}"), names_to = "year", values_to = "value") %>%
  mutate(year = as.numeric(year), state = create_scenario_label(variant),
         lab = lab_of(ifelse(grepl("ecpc", scenario_set), "ECPC", "CAPC"),
