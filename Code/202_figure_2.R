...
# lab_of, higher/lower, grid_sets are in 000_setup.R.
# Approach palette (CB-safe): ECPC blue ramp, CAPC orange ramp (1990
# darkest), Source black. ECPC 2015* = the 10-yr cooperation delay (ECPC 2015
# colour, dashed). Row order for the investment panels in lab_levels.
appr_levels <- c("ECPC 1990", "ECPC 2015", "ECPC 2015*", "ECPC 2025", "CAPC 1990", "CAPC 2015", "CAPC 2025")
appr_cols   <- c("ECPC 1990" = "#08519C", "ECPC 2015" = "#3182BD", "ECPC 2015*" = "#3182BD",
                 "ECPC 2025" = "#74A9CF", "CAPC 1990" = "#8C2D04", "CAPC 2015" = "#EC7014",
                 "CAPC 2025" = "#FE9929", "Source" = "black")
appr_lty <- setNames(rep("solid", length(appr_levels) + 1L), c(appr_levels, "Source"))
...
sl <- scenario_sets_raw %>%
  ...
  mutate(year = as.numeric(year), state = create_scenario_label(variant),
         ...
         lab = factor(lab_of(principle, start), levels = appr_levels))
...
sl_d <- scenario_sets_raw %>%
  ...
  mutate(year = as.numeric(year), state = create_scenario_label(variant),
         ...
         lab = factor("ECPC 2015*", levels = appr_levels))

# x-axis for the timeseries/benchmark panels: decadal from 2030, label 2030 & 2100
path_xbreaks <- seq(2030, 2100, 10)
path_xlabels <- function(b) ifelse(b %in% c(2030, 2100), as.character(b), "")

# ============================================================================
# ROW 1. Global Net + Gross CO2 — % change vs 2020 (the invariant-envelope anchor)
# ============================================================================
# Net + Gross CO2 by responsibility group (World invariant anchor + Higher/Lower,
# where the approaches actually diverge), lowest-f. per approach + one Source line.
emis_raw <- bind_rows(sl, sl_d) %>%
  filter(region %in% c("World", higher, lower),
  ...
  mutate(metric = ifelse(grepl("Gross", variable), "Gross CO2", "Net CO2"),
         grp = ifelse(region == "World", "World",
                      ifelse(region %in% higher, "Higher resp.", "Lower resp.")),
...
emis <- bind_rows(
  ...
  mutate(metric = factor(metric, levels = c("Net CO2", "Gross CO2")),
         ...
         lab = factor(lab, levels = c(appr_levels, "Source")))
...
p_emis <- ggplot(emis, aes(year, pct, colour = lab, linetype = lab, group = lab)) +
  ...
  scale_colour_manual(values = appr_cols, name = NULL, guide = "none") +
...
# ============================================================================
# ROW 2 (left). Fair-share transfers: cumulative finance moved,
# unlimited -> lowest-f. Transfers|Finance (billion US$2010/yr) as period-weighted
# annuity NPV (period_npv, base 2025) -> trillion US$ NPV. Colour = approach
# (ties to a/c); delay has both corners -> full dumbbell.
# ============================================================================
state_shapes <- c("Source" = 4, "Unlimited (U)" = 24, "Lowest-f. (L)" = 25)
corners <- c("Unlimited (U)", "Lowest-f. (L)")
coop_of <- function(df) df %>%
  filter(variable == "Transfers|Finance", region %in% c(higher, lower), state %in% corners,
         year >= 2030, year <= 2100) %>%
...
transfers_df <- bind_rows(
  ...
  mutate(lab = factor(lab, levels = lab_levels), state = factor(state, levels = corners))
transfers_seg <- transfers_df %>% mutate(s = ifelse(grepl("^U", state), "U", "L")) %>%
  select(principle, lab, s, coop) %>%
  tidyr::pivot_wider(names_from = s, values_from = coop) %>% filter(!is.na(L))
...
p_tr <- ggplot(transfers_df, aes(coop, lab, colour = lab)) +
  ...
  geom_point(aes(shape = state, fill = lab, alpha = state), colour = "grey25",
             size = 2.8, stroke = 0.3) +
  scale_colour_manual(values = appr_cols, guide = "none") +
  scale_fill_manual(values = appr_cols, guide = "none") +
  ...
  theme_publication(0.56) + theme(legend.position = "bottom")

# ============================================================================
# ROW 2 (right). Global energy-investment change vs source, 2026-2050 (NPV), by
# approach, unlimited -> lowest-f. Sums the Investment|Energy Supply leaf nodes
...
# Source total despite an identical physical system), so the leaves are summed
# directly. Delay (ECPC 2015*) carried via the lab column; source = shared
# ECPC2015 source.
# ============================================================================
sl_inv <- bind_rows(sl, sl_d,
  sl %>% filter(lab == "ECPC 2015", state == "Source") %>%
    mutate(lab = factor("ECPC 2015*", levels = appr_levels)))
inv_vars <- c(
  "Investment|Energy Supply|Electricity|Solar", "Investment|Energy Supply|Electricity|Wind",
  ...
  "Investment|Energy Supply|Electricity|Nuclear", "Investment|Energy Supply|Electricity|Hydro",
  "Investment|Energy Supply|Electricity|Biomass", "Investment|Energy Supply|Electricity|Geothermal",
  "Investment|Energy Supply|Hydrogen", "Investment|Energy Supply|Electricity|Coal",
  "Investment|Energy Supply|Extraction|Coal", "Investment|Energy Supply|Electricity|Gas",
  "Investment|Energy Supply|Extraction|Gas", "Investment|Energy Supply|Electricity|Oil",
  "Investment|Energy Supply|Extraction|Oil", "Investment|Energy Supply|CO2 Transport and Storage",
  "Investment|Energy Supply|Liquids", "Investment|Energy Supply|Heat",
  "Investment|Energy Supply|Extraction|Uranium", "Investment|Energy Supply|Electricity|Other",
...
inv_raw <- sl_inv %>%
  filter(variable %in% inv_vars, region %in% c(higher, lower),
...
inv_pct <- inv_raw %>% filter(state %in% corners) %>%
  ...
  mutate(pct = (inv - src) / src * 100,
         lab = factor(lab, levels = lab_levels), state = factor(state, levels = corners))
inv_seg <- inv_pct %>% mutate(s = ifelse(grepl("^U", state), "U", "L")) %>%
  select(lab, s, pct) %>% tidyr::pivot_wider(names_from = s, values_from = pct) %>% filter(!is.na(L))
...
# ============================================================================
# ROW 3. Global sectoral benchmarks — % change vs 2020 across approaches (lowest-f.)
# ============================================================================
...
pe_fuels <- bind_rows(
    ...
    pe_fuels_raw %>% filter(state == "Source", scenario_set == "800fm_ecpc2015") %>% mutate(lab = "Source")
  ) %>%
...
elec <- bind_rows(
    ...
    elec_raw %>% filter(state == "Source", scenario_set == "800fm_ecpc2015") %>% mutate(lab = "Source")
  ) %>%
...
p_sector <- ggplot(pe, aes(year, pct, colour = lab, linetype = lab, group = lab)) +
  ...
  guides(colour = guide_legend(override.aes = list(
    linetype = unname(appr_lty[c(appr_levels, "Source")])), nrow = 2)) +
  labs(x = NULL, y = "Change vs 2020 (%)",
...
ggsave(here("Manuscript", "Figures", "202_figure_2.png"),
       plot = figure_2, width = 10, height = 11, dpi = 300, units = "in", bg = "white")
