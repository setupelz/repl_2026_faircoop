...
# prin_cols, lab_of, state_shapes, corners, region_order, grp_fill and the
# approach label orders are in 000_setup.R. delay = TRUE adds the ECPC 2015
...
cons_grp <- function(grp_regions, grp_label) {
  ...
  b <- main_ssp2("800fm_ecpc2015") %>%
    filter(variant == "Baseline", variable == "Consumption", region %in% grp_regions) %>%
...
cons_df <- bind_rows(cons_grp(higher_resp, "Higher resp."),
                     cons_grp(lower_resp, "Lower resp."),
                     cons_grp(all_regions, "World")) %>%
...
p_cons <- ggplot(cons_df, aes(pct, ypos, colour = col_key, group = interaction(lab, grp))) +
  ...
  scale_y_continuous(breaks = seq_along(levels(cons_df$lab)), labels = levels(cons_df$lab)) +
  ...
  theme(legend.position = "inside",
        legend.position.inside = c(0.99, 0.01), legend.justification.inside = c(1, 0),
...
nco2_d <- nco2 %>% filter(state %in% corners) %>%
  ...
  mutate(dco2 = co2 - src)
# transfer paid = -(net finance), positive for higher-resp payers. MER $, as a
# period-weighted annuity NPV.
fin <- sl %>%
  filter(variable == "Transfers|Finance", region %in% higher_resp, state %in% corners,
         year >= 2030, year <= 2100) %>%
  group_by(principle, start, state, year) %>%
  summarise(v = sum(value, na.rm = TRUE), .groups = "drop") %>%
  group_by(principle, start, state) %>% arrange(year) %>%
...
p_trade <- ggplot(scat, aes(dco2, paid, colour = principle)) +
  ...
  geom_text(data = ~ filter(.x, state == "Lowest-f. (L)"), aes(label = start),
            size = 5 / FIG_SCALE / .pt, show.legend = FALSE, angle = 90,
            hjust = 0, nudge_y = 0.6) +
  ...
  scale_fill_manual(values = prin_cols, name = NULL,
                    guide = guide_legend(position = "inside", order = 1,
...
# --- c. Carbon price dispersion -----------------------------------------------
# Every region's lowest-f. carbon price as a multiple of
...
cp_src <- sl %>%
  filter(variable == "Price|Carbon", region %in% all_regions,
         state == "Source", year == 2030) %>%
  group_by(principle, start) %>% summarise(src = mean(value), .groups = "drop")
cp_rel <- sl %>%
  filter(variable == "Price|Carbon", region %in% all_regions,
         state == "Lowest-f. (L)", year == 2030) %>%
  left_join(cp_src, by = c("principle", "start")) %>%
  mutate(ratio = value / src, grp = ifelse(region %in% higher_resp, "Higher resp.", "Lower resp."),
         lab = factor(lab_of(principle, start), levels = lab_levels),
         ypos = as.integer(lab) + ifelse(region %in% higher_resp, 0.15, -0.15))
...
band_c <- data.frame(i = seq(2, length(region_order), 2))
...
p_cprice <- ggplot(cp_rel, aes(ratio, region, fill = grp)) +
  ...
  scale_x_continuous(transform = "log2", breaks = c(0.25, 1, 4), labels = c("0.25×", "1×", "4×"),
...
# --- d. World energy-investment shift -----------------------------------------
# Lowest-f. vs source by tech, summed across all
...
inv_world <- sl %>%
  ...
  mutate(cat = inv_tech_cat[variable]) %>%
  group_by(principle, start, state, cat, year) %>%
  summarise(v = sum(value, na.rm = TRUE), .groups = "drop") %>%
...
inv_world_realloc <- inv_world %>% pivot_wider(names_from = state, values_from = invv) %>%
  ...
  filter(!is.na(delta))
# fill the net marker by principle, as in the other panels
inv_world_net <- inv_world_realloc %>% group_by(principle, lab) %>%
  summarise(net = sum(delta), .groups = "drop") %>%
  mutate(pcol = prin_cols[principle])

p_world_inv <- ggplot(inv_world_realloc, aes(delta, lab, fill = cat)) +
  geom_vline(xintercept = 0, colour = "grey60", linewidth = 0.3) +
  geom_col(width = 0.7) +
  geom_point(data = inv_world_net, aes(net, lab), inherit.aes = FALSE,
             shape = 25, fill = inv_world_net$pcol, colour = "grey25",
             size = 2.8, stroke = 0.3) +
...
# --- Assemble -----------------------------------------------------------------
# Row 1 a consumption | b trade-off; row 2 c carbon price (full width);
