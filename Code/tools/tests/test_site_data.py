...
def test_overlay_low_marker():
    ...
    # the comparison run reports a subset of the card variables
    scen = {k for k in b.INDICATORS if not k.startswith("transfers")}
    assert set(o["indicators"]) <= scen
    assert {"total_co2", "total_ghg", "coal", "oil", "gas", "energy_co2", "non_co2"} <= set(o["indicators"])
...
def test_overlay_regional_covers_regions_and_groups():
    ...
    scen = {k for k in b.INDICATORS if not k.startswith("transfers")}
    assert set(r["CHN"]) <= scen
    assert {"total_co2", "coal", "oil", "gas", "energy_co2", "non_co2"} <= set(r["CHN"])
